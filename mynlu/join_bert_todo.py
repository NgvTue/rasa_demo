...
def load_model(path=None, cache_dir=None):
    ...
    phobert = AutoModel.from_pretrained(path, cache_dir=cache_dir)
...
class Bert(DenseFeaturizer):
    ...
    defaults = {"path": None, "device": 'cpu',"cache_dir":None,BILOU_FLAG: True,FEATURIZERS: [],
    ...
    def __init__(self, component_config: Optional[Dict[Text, Any]] = None) -> None:
        ...
        cache_dir = component_config.get("cache_dir",None)
    ...
    def gen_feature(self, message, attribute = TEXT):
        ...
        if not tokens:
            ...
            return None
        logging.debug("tokens is none at process")
        logging.debug(f"{[i.text for i in tokens]}")
